 ...
 * Graph support in SQL Server 2017 addresses this by introducing simple constructs of nodes
 * and relationships that enable sophisticated modeling.
 *
...
INSERT INTO MONITORS VALUES(
	(SELECT $NODE_ID FROM SENSOR WHERE ID = 1),
	(SELECT $NODE_ID FROM MACHINE WHERE ID =1), 1 );
INSERT INTO MONITORS VALUES(
	(SELECT $NODE_ID FROM SENSOR WHERE ID = 2),
	(SELECT $NODE_ID FROM MACHINE WHERE ID =2), 1 );
...
--
-- Question #1: which sensors are reporting failure? Which machines are affected?
...
--
-- Question #2: which machines are affected by the failure? Production for which products is affected?
...
--
-- Question #3: which factory produces the products whose production is affected by the machine failure?
-- Traverse graph: Sensor -> Monitors -> Machine -> Reports -> Failure -> Affects -> Machine -> Assemblies -> Product
